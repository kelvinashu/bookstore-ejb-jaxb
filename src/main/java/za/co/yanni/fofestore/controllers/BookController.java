package za.co.yanni.fofestore.controllers;

import za.co.yanni.fofestore.entities.Book;
import za.co.yanni.fofestore.controllers.util.JsfUtil;
import za.co.yanni.fofestore.controllers.util.PaginationHelper;
import za.co.yanni.fofestore.sessionbeans.BookFacade;
...
import za.co.yanni.fofestore.controllers.util.InitializeStore;
import za.co.yanni.fofestore.entities.Author;
import za.co.yanni.fofestore.jaxb.Authors;
...
public class BookController implements Serializable {
    ...
    @EJB
    private za.co.yanni.fofestore.sessionbeans.BookFacade ejbFacade;
    ...
    public String prepareList() {
        recreateModel();
        return "/pages/book/List.xhtml";
    }

    public String loadBooks() {
        Bookstore bookstore = InitializeStore.bookstore();
        if (bookstore != null) {

            for (za.co.yanni.fofestore.jaxb.Book book : bookstore.getBook()) {

                Book bookObj = new Book();

                bookObj.setCategory(book.getCategory());
                bookObj.setLang(book.getTitle().getLang());
                bookObj.setTitle(book.getTitle().getValue());
                bookObj.setPrice(book.getPrice());
                bookObj.setYear(book.getYear());

                System.out.println("BOOK DB: " + bookObj);

                Author authorObj = new Author();

                //add author to book
                if (book.getAuthors() != null) {
                    if (book.getAuthors().getAuthor() != null) {
                        for (String author : book.getAuthors().getAuthor()) {
                            authorObj.setName(author);
                            System.out.println("Author DB: " + authorObj);
                        }
                    }
                }

                ejbFacade.create(bookObj);
                System.out.println("AFTER COMMIT : " + bookObj);
            }
